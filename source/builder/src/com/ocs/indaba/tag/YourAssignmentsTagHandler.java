 ...
 * @author Jeff Jiang
 */
public class YourAssignmentsTagHandler extends BaseTagHandler {

    private static final Logger logger = Logger.getLogger(YourAssignmentsTagHandler.class);
    private static final String TASK_DISPLAY_PATTERN = "{0} - {1} {2}";
    
    //private static final String IMG_STATUS_ACTIVE = "status_black.png";
    //private static final String IMG_STATUS_AWARE = "status_pink.png";
    //private static final String IMG_STATUS_NOTICED = "status_blue.png";
    //private static final String IMG_STATUS_STARTED = "status_darkgoldenrod.png";
    ...
    public int doStartTag() {
        ...
        // if subjectUid is not specified, just simply assume subjectUid is the same as targetUid
        if (subjectUid <= 0) {
            subjectUid = targetUid;
        }

        if (subjectUid == targetUid) {
            assignedTasks = taskService.getAssignedTasksByUserId(targetUid, prjid);
        } else {
        ...
        if (assignedTasks != null && assignedTasks.size() > 0) {
            try {
                ...
                for (AssignedTask task : assignedTasks) {
                    ...
                    task.setTaskStatusIcon(getLanguageId());
    ...
    private void outputTask(JspWriter out, AssignedTask task) throws IOException {
        ...
        out.print("<td>");
        if (hasEditDeadlineRight && task.getStatus() != Constants.TASK_STATUS_DONE) {
            out.print("&nbsp;&nbsp;&nbsp;<a id='inline' href='#deadline' title='"
                    + getI18nMessage(Messages.KEY_COMMON_BUTTON_EDITDEADLINE)
                    +"' onclick='setAssignment(\"" + task.getAssignmentId() +
                    "\",\"" + taskDisplay + "\",\"" + task.getDisplayDeadline() + "\",\"" + StringEscapeUtils.escapeXml(userName) + "\");'>" + 
