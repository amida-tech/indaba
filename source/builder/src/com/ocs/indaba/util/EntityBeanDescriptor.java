...
public class EntityBeanDescriptor {
    ...
    private final Map<String, FieldDescriptor> aliasDescriptorMap = new HashMap<String, FieldDescriptor>();
    private final Map<String, FieldDescriptor> columnDescriptorMap = new HashMap<String, FieldDescriptor>();
    private final Map<String, FieldDescriptor> filedDescriptorMap = new HashMap<String, FieldDescriptor>();
        ...
        for (FieldDescriptor fd : this.fieldDescriptors) {
            ...
            if (!columnDescriptorMap.get(columnName).isManyToMany() && !columnDescriptorMap.get(columnName).isOneToMany()) {
                if (!columnName.equalsIgnoreCase(primaryKeyName) || !isPKIncremental()) {
                    insertClausePrefix.append("`").append(columnName).append("`");
                    insertClausePrefix.append(",");

                    valuesClause.append("?,");
                }

                selectClausePrefix.append("`").append(columnName).append("`,");

                if (!columnName.equalsIgnoreCase(primaryKeyName)) {
                    updateClausePrefix.append("`").append(columnName).append("`=?,");
                }
            }
        }
    ...
    public StringBuilder getInsertWithIdClausePrefix() {
        StringBuilder sBuf = new StringBuilder();
        int i = insertClausePrefix.indexOf("(");
        sBuf.append(insertClausePrefix.substring(0, i + 1));
        sBuf.append("id,");
        int j = insertClausePrefix.lastIndexOf("(");
        sBuf.append(insertClausePrefix.substring(i + 1, j + 1));
        sBuf.append("?,");
        sBuf.append(insertClausePrefix.substring(j + 1));
        return sBuf;
    }
    ...
    public int[] getColumnWithIdSqlTypes() {
        int[] arr = new int[columnSqlTypes.length + 1];
        arr[0] = Types.INTEGER;
        System.arraycopy(columnSqlTypes, 0, arr, 1, columnSqlTypes.length);
        return arr;
    }
