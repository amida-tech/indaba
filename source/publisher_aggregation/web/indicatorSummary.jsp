<html>
    <head>
        ...
        <script type="text/javascript">
            function download() {
                $.getJSON("indicatorSummaryChart.do?size=large&genericLabel=${param.genericLabel}&horseId=${param.horseId}", {t:new Date()}, function(data) {
                    window.location.href = "highResDownload.do?filename=" + data.charts.substring(data.charts.lastIndexOf("/") + 1);
                });
            }
            
            $(function() {
                $.getJSON("indicatorSummaryChart.do?size=${param.size}&genericLabel=${param.genericLabel}&horseId=${param.horseId}", {t:new Date()}, function(data) {
                    ...
                    if (data.charts == "No Data") {
                        $("<div>No Data</div>").appendTo("#chartDiv").load(function(){
                            $WIDGET.widgetResizePipe('INDICATOR_SUMMARY');});
                        $("#highRes").hide();
                    } else {
                        $("<img/>", {
                            "id"    :   "chartImg",
