 ...
 * @author luwb
 */
public class ScorecardAnswerBDAO extends SmartDaoMySqlImpl<ScorecardAnswerB, Integer> {

    private static final Logger log = Logger.getLogger(ScorecardAnswerBDAO.class);
    private static final String SELECT_SCORECARD_ANSWER_BY_SCORECARD_AND_INDICATOR_ID =
            "SELECT * from scorecard_answer_b WHERE scorecard_id=? and indicator_id IN ({0})";
    private static final String TRUNCATE_SCORECARD_ANSWER =
            "TRUNCATE scorecard_answer_b";
    private static final String SELECT_SCORECARD_ANSWER_BY_FILTER =
            "SELECT sa.*, s.org_id FROM scorecard_answer_b sa JOIN scorecard s ON (sa.scorecard_id = s.id) "
    ...
    public List<ScorecardAnswer> selectScorecardAnswerByScorecardAndIndicatorId(int scorecardId, List<Integer> indicatorIds) {
        ...
        List<ScorecardAnswerB> listB = super.find(sqlStr, scorecardId);
        List<ScorecardAnswer> list = new ArrayList<ScorecardAnswer>();
        if (listB != null) {
            for (ScorecardAnswerB scaB : listB) {
                list.add(ScorecardAnswerHelper.scaB2sca(scaB));
            }
        }
        return list;
    }
    ...
    public void deleteAll() {
        log.debug("delete all scorecard_answer_b data.");
