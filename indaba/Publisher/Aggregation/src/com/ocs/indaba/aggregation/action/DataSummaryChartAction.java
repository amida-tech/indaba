...
import com.ocs.indaba.aggregation.tool.DataSummaryChartGenerator;
 ...
 * @author Jeanbone
 */
public class DataSummaryChartAction extends BaseExportAction {

    private static final Logger log = Logger.getLogger(DataSummaryChartAction.class);
    private static String PARAM_HORSE_ID = "horseId";
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        log.info("Get data summary chart - [horseId=" + horseId + ", version=" + version + " baseDir=" + baseDir
                + ", cacheDir=" + cacheDir + "] - " + request.getParameter("genericLabel") + ":" + genericLabel);

        StringBuilder sb = new StringBuilder();
        sb.append("dataSummary_h").append(horseId);
        if (genericLabel) {
            sb.append("_g");
        }
        if (isLarge) {
        ...
        JSONObject jsonDataSummary = DataSummaryJsonUtils.toJson(horseId, baseDir, cacheDir, genericLabel);
        if (jsonDataSummary != null) {
            String tempPath = fileDir.concat(UUID.randomUUID().toString()).concat(".jpg");
            JfreeChartGenerator chartGen = new DataSummaryChartGenerator(tempPath, jsonDataSummary, isLarge, genericLabel);
