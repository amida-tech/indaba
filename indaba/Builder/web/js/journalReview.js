...
function loadStaffDiscussions(param) {
    ...
    parameters.msgboardId = msgboardId;
    parameters.folded = param.folded;
    parameters.discussionType='JOURNAL_STAFF_AUTHOR_DISCUSSION';
    if (param.syncStatus == "1" && param.assignId != "0") {
        parameters.syncStatus = true;
        parameters.syncStatusFun = function() {
            updateStatusAndPercentage(param.ctxPath, param.assignId, 4, 0.5);
...
function loadPeerReviews(param) {
    ...
    $("button[name=SaveOpinions]").live("click", function() {
        ...
        $.ajax({
            ...
            success: function(result) {
				...
				*/
                if (result == "OK") {
                    jInfo($.i18n.message('common.js.alert.succssreview'), $.i18n.message('common.js.alert.title.success'), null);
                }
            },
            error: function(result) {
                alert(result);
    ...
    parameters.discussionType='JOURNAL_PEER_REVIEW_DISCUSSIONS';
    if (param.syncStatus == "1" && param.assignId != "0") {
        parameters.syncStatus = true;
        parameters.syncStatusFun = function() {
            updateStatusAndPercentage(param.ctxPath, param.assignId, 4, 0.5);
...
function loadPRReviews(param) {
    ...
    for (var i = 0; i < peerReviews.length; i++) {
        ...
        "<h4>"+$.i18n.message('common.label.reviewTips')+":</h4>" +
        "<textarea " + (editable ? "" : "readonly=\"true\"") + ">" + (peerReview.opinions == undefined ? "" : peerReview.opinions) + "</textarea>" +
        (editable ? "<button style=\"float:right; margin: 5px 70px;\" class=\"add medium button blue icon-save\" name=\"SaveOpinions" + peerReview.id + "\">"+$.i18n.message('common.btn.save')+"</button>" : "") +
        "<div class=\"clear\"></div>" +
        "</td></tr></table></div>" +
...
function iHaveQuestions(ctx, horseid, assignid) {
    var prompt = $.i18n.message('common.js.alert.sumitquestion');
    jConfirm(prompt, $.i18n.message('common.js.alert.title.confirm'), function(r) {
        if (r) {
            var parameters = new Object();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response == "OK") {
                ...
                msg = $.i18n.message('common.js.alert.incompletedquestion');
            }
