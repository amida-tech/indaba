...
(function() {
	...
	//fr
	var phrases_fr = {
			'progressOverall': 'Progression générale ({total})',
			'currentTitle': 'progrès fichier',
			'currentFile': 'téléchargement "{name}"',
			'currentProgress': 'Upload: {bytesLoaded} avec {rate}, {timeRemaining} restant.',
			'fileName': '{name}',
			'remove': 'supprimer',
			'removeTitle': 'Cliquez ici pour supprimer cette entrée.',
			'fileError': 'échec de l\'envoi',
			'validationErrors': {
				'duplicate': 'dossier <em>{name}</em> is already added, duplicates are not allowed.',
				'sizeLimitMin': 'dossier <em>{name}</em> (<em>{size}</em>) est trop petit, la taille du fichier minimal est {fileSizeMin}.',
				'sizeLimitMax': 'dossier <em>{name}</em> (<em>{size}</em>) est trop grand, la taille du fichier maximale est de <em>{fileSizeMax}</em>.',
				'fileListMax': 'dossier <em>{name}</em> ne peut être ajouté, la quantité de <em>{fileListMax} fichiers</em> dépassé.',
				'fileListSizeMax': 'dossier <em>{name}</em> (<em>{size}</em>) est trop grand, la taille du fichier global de <em>{fileListSizeMax}</em> dépassé.'
			},
			'errors': {
				'httpStatus': 'Serveur HTTP renvoyé inscrits <code>#{code}</code>',
				'securityError': 'Erreur de sécurité s\'est produite ({text})',
				'ioError': 'Erreur causée un envoi ou d\'un chargement à l\'échec ({text})'
			}
		};
		MooTools.fancyUploadI18N = new Hash({en: phrases});
		MooTools.fancyUploadI18N.set('fr', phrases_fr);
	...
	if (MooTools.lang) {
		...
		MooTools.lang = {
			get: function(from, key) {
				var alerts = MooTools.fancyUploadI18N.get(jQuery.i18n.message('common.language'));
				if(alerts){
					return alerts[key];
				}
				return phrases[key];
